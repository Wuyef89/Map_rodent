...
write.csv(test_table, "BRT/OUTPUT/HFRS/TEST/test3.csv")
...
writeRaster(HFRS.risk, 
            filename = "BRT/OUTPUT/HFRS/TIF/HFRS_risk3.tif", overwrite = TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution3.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image3.RData")
...
#1
GDP3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GDP"), model = i)
})
GDP3 <- do.call(rbind, GDP3)

#2
Elevation3 <- lapply(1:length(model_list), function(i){
  data.frame(getVar(model_list[[i]],"Elevation"), model = i)
})
Elevation3 <- do.call(rbind, Elevation3)

#3
Barren3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Barren"), model = i)
})
Barren3 <- do.call(rbind, Barren3)

#4
PRS3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS3 <- do.call(rbind, PRS3)

#5
MammalRichness3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"MammalRichness"), model = i)
})
MammalRichness3 <- do.call(rbind, MammalRichness3)

#6
SSD3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD3 <- do.call(rbind, SSD3)

#7
PRE3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE3 <- do.call(rbind, PRE3)

#8
WIN3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN3 <- do.call(rbind, WIN3)

#9
RHU3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU3 <- do.call(rbind, RHU3)

#3
GST3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST3 <- do.call(rbind, GST3)


write.csv(GDP3,"BRT/OUTPUT/HFRS/VAR/GDP3.csv")
write.csv(Elevation3,"BRT/OUTPUT/HFRS/VAR/Elevation3.csv")
write.csv(Barren3,"BRT/OUTPUT/HFRS/VAR/Barren3.csv")
write.csv(PRS3,"BRT/OUTPUT/HFRS/VAR/PRS3.csv")
write.csv(MammalRichness3,"BRT/OUTPUT/HFRS/VAR/MammalRichness3.csv")
write.csv(SSD3,"BRT/OUTPUT/HFRS/VAR/SSD3.csv")
write.csv(PRE3,"BRT/OUTPUT/HFRS/VAR/PRE3.csv")
write.csv(WIN3,"BRT/OUTPUT/HFRS/VAR/WIN3.csv")
write.csv(RHU3,"BRT/OUTPUT/HFRS/VAR/RHU3.csv")
write.csv(GST3,"BRT/OUTPUT/HFRS/VAR/GST3.csv")
