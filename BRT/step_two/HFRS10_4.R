...
write.csv(test_table, "BRT/OUTPUT/HFRS/TEST/test4.csv")
...
writeRaster(HFRS.risk, 
            filename = "BRT/OUTPUT/HFRS/TIF/HFRS_risk4.tif", overwrite = TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution4.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image4.RData")
...
#1
GDP4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GDP"), model = i)
})
GDP4 <- do.call(rbind, GDP4)

#2
Elevation4 <- lapply(1:length(model_list), function(i){
  data.frame(getVar(model_list[[i]],"Elevation"), model = i)
})
Elevation4 <- do.call(rbind, Elevation4)

#3
Barren4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Barren"), model = i)
})
Barren4 <- do.call(rbind, Barren4)

#4
PRS4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS4 <- do.call(rbind, PRS4)

#5
MammalRichness4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"MammalRichness"), model = i)
})
MammalRichness4 <- do.call(rbind, MammalRichness4)

#6
SSD4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD4 <- do.call(rbind, SSD4)

#7
PRE4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE4 <- do.call(rbind, PRE4)

#8
WIN4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN4 <- do.call(rbind, WIN4)

#9
RHU4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU4 <- do.call(rbind, RHU4)

#4
GST4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST4 <- do.call(rbind, GST4)


write.csv(GDP4,"BRT/OUTPUT/HFRS/VAR/GDP4.csv")
write.csv(Elevation4,"BRT/OUTPUT/HFRS/VAR/Elevation4.csv")
write.csv(Barren4,"BRT/OUTPUT/HFRS/VAR/Barren4.csv")
write.csv(PRS4,"BRT/OUTPUT/HFRS/VAR/PRS4.csv")
write.csv(MammalRichness4,"BRT/OUTPUT/HFRS/VAR/MammalRichness4.csv")
write.csv(SSD4,"BRT/OUTPUT/HFRS/VAR/SSD4.csv")
write.csv(PRE4,"BRT/OUTPUT/HFRS/VAR/PRE4.csv")
write.csv(WIN4,"BRT/OUTPUT/HFRS/VAR/WIN4.csv")
write.csv(RHU4,"BRT/OUTPUT/HFRS/VAR/RHU4.csv")
write.csv(GST4,"BRT/OUTPUT/HFRS/VAR/GST4.csv")
