...
write.csv(test_table, "BRT/OUTPUT/HFRS/TEST/test1.csv")
...
writeRaster(HFRS.risk, 
            filename = "BRT/OUTPUT/HFRS/TIF/HFRS_risk1.tif", overwrite = TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution1.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image1.RData")
...
#1
GDP1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GDP"), model = i)
})
GDP1 <- do.call(rbind, GDP1)

#2
Elevation1 <- lapply(1:length(model_list), function(i){
  data.frame(getVar(model_list[[i]],"Elevation"), model = i)
})
Elevation1 <- do.call(rbind, Elevation1)

#3
Barren1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Barren"), model = i)
})
Barren1 <- do.call(rbind, Barren1)

#4
PRS1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS1 <- do.call(rbind, PRS1)

#5
MammalRichness1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"MammalRichness"), model = i)
})
MammalRichness1 <- do.call(rbind, MammalRichness1)

#6
SSD1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD1 <- do.call(rbind, SSD1)

#7
PRE1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE1 <- do.call(rbind, PRE1)

#8
WIN1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN1 <- do.call(rbind, WIN1)

#9
RHU1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU1 <- do.call(rbind, RHU1)

#1
GST1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST1 <- do.call(rbind, GST1)


write.csv(GDP1,"BRT/OUTPUT/HFRS/VAR/GDP1.csv")
write.csv(Elevation1,"BRT/OUTPUT/HFRS/VAR/Elevation1.csv")
write.csv(Barren1,"BRT/OUTPUT/HFRS/VAR/Barren1.csv")
write.csv(PRS1,"BRT/OUTPUT/HFRS/VAR/PRS1.csv")
write.csv(MammalRichness1,"BRT/OUTPUT/HFRS/VAR/MammalRichness1.csv")
write.csv(SSD1,"BRT/OUTPUT/HFRS/VAR/SSD1.csv")
write.csv(PRE1,"BRT/OUTPUT/HFRS/VAR/PRE1.csv")
write.csv(WIN1,"BRT/OUTPUT/HFRS/VAR/WIN1.csv")
write.csv(RHU1,"BRT/OUTPUT/HFRS/VAR/RHU1.csv")
write.csv(GST1,"BRT/OUTPUT/HFRS/VAR/GST1.csv")
