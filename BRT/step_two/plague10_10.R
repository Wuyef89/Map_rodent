...
for (i in seq_along(provinces)) {
  ...
  if (matching_row$plague == 0) {
    province_df$Abspres <- 0
  } else if (nrow(province_df) < matching_row$plague) {
    province_df$Abspres <- 1
  } else {
    num_positive <- matching_row$plague
...
colnames(model_data)
model_data<-model_data[,c(11,9,21,8,4,7,22,14,17,6,15,25)] # Variables selected based on 5-fold cross-validation
colnames(model_data)
...
write.csv(test_table,"BRT/OUTPUT/PLAGUE/TEST/test10.csv")

#Write the average tif
plague.risk <- rast(predictors)  
plague.risk[] <- 0  
for (i in seq_along(model_list)) {
  risk <- predict(predictors, model_list[[i]], n.trees=model_list[[i]]$gbm.call$best.trees, type="response")
  plague.risk <- plague.risk + risk 
}
plague.risk <- plague.risk / length(model_list)
writeRaster(plague.risk, 
            filename = "BRT/OUTPUT/PLAGUE/TIF/plague_risk10.tif", overwrite=TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/PLAGUE/CONTRIBUTION/contribution10.csv")
save.image("BRT/OUTPUT/PLAGUE/IMAGE/image10.RData")
...
#5
DNT10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"DNT"), model = i)
})
DNT10 <- do.call(rbind, DNT10)

#6
EVP10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"EVP"), model = i)
})
EVP10 <- do.call(rbind, EVP10)

#7
TEM10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"TEM"), model = i)
})
TEM10 <- do.call(rbind, TEM10)
...
write.csv(MammalRichness10,"BRT/OUTPUT/PLAGUE/VAR/MammalRichness10.csv")
write.csv(GST10,"BRT/OUTPUT/PLAGUE/VAR/GST10.csv")
write.csv(SSD10,"BRT/OUTPUT/PLAGUE/VAR/SSD10.csv")
write.csv(GDP10,"BRT/OUTPUT/PLAGUE/VAR/GDP10.csv")
write.csv(DNT10,"BRT/OUTPUT/PLAGUE/VAR/DNT10.csv")
write.csv(EVP10,"BRT/OUTPUT/PLAGUE/VAR/EVP10.csv")
write.csv(TEM10,"BRT/OUTPUT/PLAGUE/VAR/TEM10.csv")
write.csv(PRE10,"BRT/OUTPUT/PLAGUE/VAR/PRE10.csv")
write.csv(RHU10,"BRT/OUTPUT/PLAGUE/VAR/RHU10.csv")
write.csv(Elevation10,"BRT/OUTPUT/PLAGUE/VAR/Elevation10.csv")
write.csv(PRS10,"BRT/OUTPUT/PLAGUE/VAR/PRS10.csv")
