...
write.csv(test_table, "BRT/OUTPUT/HFRS/TEST/test7.csv")
...
writeRaster(HFRS.risk, 
            filename = "BRT/OUTPUT/HFRS/TIF/HFRS_risk7.tif", overwrite = TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution7.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image7.RData")
...
#1
GDP7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GDP"), model = i)
})
GDP7 <- do.call(rbind, GDP7)

#2
Elevation7 <- lapply(1:length(model_list), function(i){
  data.frame(getVar(model_list[[i]],"Elevation"), model = i)
})
Elevation7 <- do.call(rbind, Elevation7)

#3
Barren7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Barren"), model = i)
})
Barren7 <- do.call(rbind, Barren7)

#4
PRS7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS7 <- do.call(rbind, PRS7)

#5
MammalRichness7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"MammalRichness"), model = i)
})
MammalRichness7 <- do.call(rbind, MammalRichness7)

#6
SSD7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD7 <- do.call(rbind, SSD7)

#7
PRE7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE7 <- do.call(rbind, PRE7)

#8
WIN7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN7 <- do.call(rbind, WIN7)

#9
RHU7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU7 <- do.call(rbind, RHU7)

#7
GST7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST7 <- do.call(rbind, GST7)


write.csv(GDP7,"BRT/OUTPUT/HFRS/VAR/GDP7.csv")
write.csv(Elevation7,"BRT/OUTPUT/HFRS/VAR/Elevation7.csv")
write.csv(Barren7,"BRT/OUTPUT/HFRS/VAR/Barren7.csv")
write.csv(PRS7,"BRT/OUTPUT/HFRS/VAR/PRS7.csv")
write.csv(MammalRichness7,"BRT/OUTPUT/HFRS/VAR/MammalRichness7.csv")
write.csv(SSD7,"BRT/OUTPUT/HFRS/VAR/SSD7.csv")
write.csv(PRE7,"BRT/OUTPUT/HFRS/VAR/PRE7.csv")
write.csv(WIN7,"BRT/OUTPUT/HFRS/VAR/WIN7.csv")
write.csv(RHU7,"BRT/OUTPUT/HFRS/VAR/RHU7.csv")
write.csv(GST7,"BRT/OUTPUT/HFRS/VAR/GST7.csv")
