...
write.csv(test_table,"BRT/OUTPUT/TYPHUS/TEST/test5.csv")
...
writeRaster(typhus.risk, 
            filename = "BRT/OUTPUT/TYPHUS/TIF/typhus_risk5.tif", overwrite=TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/TYPHUS/CONTRIBUTION/contribution5.csv")
save.image("BRT/OUTPUT/TYPHUS/IMAGE/image5.RData")
...
Elevation5 <- do.call(cbind, Elevation5)
...
MammalRichness5 <- do.call(cbind, MammalRichness5)

#4
SSD5 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD5 <- do.call(cbind, SSD5)

#5
PRS5 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS5 <- do.call(cbind, PRS5)

#6
PRE5 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE5 <- do.call(cbind, PRE5)

#7
RHU5 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU5 <- do.call(cbind, RHU5)

#8
GST5 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST5 <- do.call(cbind, GST5)

#9
EBT5 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"EBT"), model = i)
})
EBT5 <- do.call(cbind, EBT5)

#10
WIN5 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN5 <- do.call(cbind, WIN5)

#11
Shrubs5 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Shrubs"), model = i)
})
Shrubs5 <- do.call(cbind, Shrubs5)


write.csv(Elevation5,"BRT/OUTPUT/TYPHUS/VAR//Elevation5.csv")
write.csv(GDP5,"BRT/OUTPUT/TYPHUS/VAR//GDP5.csv")
write.csv(MammalRichness5,"BRT/OUTPUT/TYPHUS/VAR//MammalRichness5.csv")
write.csv(SSD5,"BRT/OUTPUT/TYPHUS/VAR//SSD5.csv")
write.csv(PRS5,"BRT/OUTPUT/TYPHUS/VAR//PRS5.csv")
write.csv(PRE5,"BRT/OUTPUT/TYPHUS/VAR//PRE5.csv")
write.csv(RHU5,"BRT/OUTPUT/TYPHUS/VAR//RHU5.csv")
write.csv(GST5,"BRT/OUTPUT/TYPHUS/VAR//GST5.csv")
write.csv(EBT5,"BRT/OUTPUT/TYPHUS/VAR//EBT5.csv")
write.csv(WIN5,"BRT/OUTPUT/TYPHUS/VAR//WIN5.csv")
write.csv(Shrubs5,"BRT/OUTPUT/TYPHUS/VAR//Shrubs5.csv")
