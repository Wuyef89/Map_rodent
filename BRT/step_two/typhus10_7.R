...
write.csv(test_table,"BRT/OUTPUT/TYPHUS/TEST/test7.csv")
...
writeRaster(typhus.risk, 
            filename = "BRT/OUTPUT/TYPHUS/TIF/typhus_risk7.tif", overwrite=TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/TYPHUS/CONTRIBUTION/contribution7.csv")
save.image("BRT/OUTPUT/TYPHUS/IMAGE/image7.RData")
...
Elevation7 <- do.call(cbind, Elevation7)
...
MammalRichness7 <- do.call(cbind, MammalRichness7)

#4
SSD7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD7 <- do.call(cbind, SSD7)

#5
PRS7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS7 <- do.call(cbind, PRS7)

#6
PRE7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE7 <- do.call(cbind, PRE7)

#7
RHU7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU7 <- do.call(cbind, RHU7)

#8
GST7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST7 <- do.call(cbind, GST7)

#9
EBT7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"EBT"), model = i)
})
EBT7 <- do.call(cbind, EBT7)

#10
WIN7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN7 <- do.call(cbind, WIN7)

#11
Shrubs7 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Shrubs"), model = i)
})
Shrubs7 <- do.call(cbind, Shrubs7)


write.csv(Elevation7,"BRT/OUTPUT/TYPHUS/VAR//Elevation7.csv")
write.csv(GDP7,"BRT/OUTPUT/TYPHUS/VAR//GDP7.csv")
write.csv(MammalRichness7,"BRT/OUTPUT/TYPHUS/VAR//MammalRichness7.csv")
write.csv(SSD7,"BRT/OUTPUT/TYPHUS/VAR//SSD7.csv")
write.csv(PRS7,"BRT/OUTPUT/TYPHUS/VAR//PRS7.csv")
write.csv(PRE7,"BRT/OUTPUT/TYPHUS/VAR//PRE7.csv")
write.csv(RHU7,"BRT/OUTPUT/TYPHUS/VAR//RHU7.csv")
write.csv(GST7,"BRT/OUTPUT/TYPHUS/VAR//GST7.csv")
write.csv(EBT7,"BRT/OUTPUT/TYPHUS/VAR//EBT7.csv")
write.csv(WIN7,"BRT/OUTPUT/TYPHUS/VAR//WIN7.csv")
write.csv(Shrubs7,"BRT/OUTPUT/TYPHUS/VAR//Shrubs7.csv")
