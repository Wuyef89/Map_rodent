...
write.csv(test_table,"BRT/OUTPUT/TYPHUS/TEST/test4.csv")
...
writeRaster(typhus.risk, 
            filename = "BRT/OUTPUT/TYPHUS/TIF/typhus_risk4.tif", overwrite=TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/TYPHUS/CONTRIBUTION/contribution4.csv")
save.image("BRT/OUTPUT/TYPHUS/IMAGE/image4.RData")
...
Elevation4 <- do.call(cbind, Elevation4)
...
MammalRichness4 <- do.call(cbind, MammalRichness4)

#4
SSD4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD4 <- do.call(cbind, SSD4)

#5
PRS4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS4 <- do.call(cbind, PRS4)

#6
PRE4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE4 <- do.call(cbind, PRE4)

#7
RHU4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU4 <- do.call(cbind, RHU4)

#8
GST4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST4 <- do.call(cbind, GST4)

#9
EBT4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"EBT"), model = i)
})
EBT4 <- do.call(cbind, EBT4)

#10
WIN4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN4 <- do.call(cbind, WIN4)

#11
Shrubs4 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Shrubs"), model = i)
})
Shrubs4 <- do.call(cbind, Shrubs4)


write.csv(Elevation4,"BRT/OUTPUT/TYPHUS/VAR//Elevation4.csv")
write.csv(GDP4,"BRT/OUTPUT/TYPHUS/VAR//GDP4.csv")
write.csv(MammalRichness4,"BRT/OUTPUT/TYPHUS/VAR//MammalRichness4.csv")
write.csv(SSD4,"BRT/OUTPUT/TYPHUS/VAR//SSD4.csv")
write.csv(PRS4,"BRT/OUTPUT/TYPHUS/VAR//PRS4.csv")
write.csv(PRE4,"BRT/OUTPUT/TYPHUS/VAR//PRE4.csv")
write.csv(RHU4,"BRT/OUTPUT/TYPHUS/VAR//RHU4.csv")
write.csv(GST4,"BRT/OUTPUT/TYPHUS/VAR//GST4.csv")
write.csv(EBT4,"BRT/OUTPUT/TYPHUS/VAR//EBT4.csv")
write.csv(WIN4,"BRT/OUTPUT/TYPHUS/VAR//WIN4.csv")
write.csv(Shrubs4,"BRT/OUTPUT/TYPHUS/VAR//Shrubs4.csv")
