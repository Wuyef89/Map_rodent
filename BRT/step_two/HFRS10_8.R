...
write.csv(test_table, "BRT/OUTPUT/HFRS/TEST/test8.csv")
...
writeRaster(HFRS.risk, 
            filename = "BRT/OUTPUT/HFRS/TIF/HFRS_risk8.tif", overwrite = TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution8.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image8.RData")
...
Elevation8 <- do.call(rbind, Elevation8)

#3
Barren8 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Barren"), model = i)
})
Barren8 <- do.call(rbind, Barren8)

#4
PRS8 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS8 <- do.call(rbind, PRS8)

#5
MammalRichness8 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"MammalRichness"), model = i)
})
MammalRichness8 <- do.call(rbind, MammalRichness8)

#6
SSD8 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD8 <- do.call(rbind, SSD8)

#7
PRE8 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE8 <- do.call(rbind, PRE8)

#8
WIN8 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN8 <- do.call(rbind, WIN8)

#9
RHU8 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU8 <- do.call(rbind, RHU8)

#10
GST8 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST8 <- do.call(rbind, GST8)


write.csv(GDP8,"BRT/OUTPUT/HFRS/VAR/GDP8.csv")
write.csv(Elevation8,"BRT/OUTPUT/HFRS/VAR/Elevation8.csv")
write.csv(Barren8,"BRT/OUTPUT/HFRS/VAR/Barren8.csv")
write.csv(PRS8,"BRT/OUTPUT/HFRS/VAR/PRS8.csv")
write.csv(MammalRichness8,"BRT/OUTPUT/HFRS/VAR/MammalRichness8.csv")
write.csv(SSD8,"BRT/OUTPUT/HFRS/VAR/SSD8.csv")
write.csv(PRE8,"BRT/OUTPUT/HFRS/VAR/PRE8.csv")
write.csv(WIN8,"BRT/OUTPUT/HFRS/VAR/WIN8.csv")
write.csv(RHU8,"BRT/OUTPUT/HFRS/VAR/RHU8.csv")
write.csv(GST8,"BRT/OUTPUT/HFRS/VAR/GST8.csv")
