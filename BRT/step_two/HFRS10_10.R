...
for (i in seq_along(provinces)) {
  ...
  if (matching_row$HFRS == 0) {
    province_df$Abspres <- 0
  } else if (nrow(province_df) < matching_row$HFRS) {
    province_df$Abspres <- 1
  } else {
    num_positive <- matching_row$HFRS
...
tif_file_name <- list.files("BRT/DATA/ResamplePost/Model/TrainCases", 
                            pattern = "\\.tif$", full.names = TRUE, ignore.case = TRUE)
predictors_file_name <- list.files("BRT/DATA/ResamplePost/Model/PredictorsCases", 
                                   pattern = "\\.tif$", full.names = TRUE, ignore.case = TRUE)
...
colnames(model_data)
model_data <- model_data[,c(8,6,1,15,11,21,14,24,17,9,25)] # Variables selected based on 5-fold cross-validation
colnames(model_data)

# Create a table to store the predictions
test_table <- point
test_table$preds <- NA  # Add a new column preds to test_table and initialize all values to NA

# Build models and calculate AUC
model_list <- list()  # Create an empty list, possibly to store a series of model objects later
folds <- scv1$folds_list # Extract the element named folds_list from scv1 and assign it to the variable folds
...
write.csv(test_table, "BRT/OUTPUT/HFRS/TEST/test10.csv")

# Write the average tif
HFRS.risk <- rast(predictors)  # Use the same dimensions as predictors (number of rows, columns, and layers)
HFRS.risk[] <- 0  # Initialize all values to 0
for (i in seq_along(model_list)) {
  risk <- predict(predictors, model_list[[i]], n.trees = model_list[[i]]$gbm.call$best.trees, type = "response")
  HFRS.risk <- HFRS.risk + risk # Accumulate the prediction results (risk) of the current model to the previous results (HFRS.risk)
}
HFRS.risk <- HFRS.risk / length(model_list)
writeRaster(HFRS.risk, 
            filename = "BRT/OUTPUT/HFRS/TIF/HFRS_risk10.tif", overwrite = TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution10.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image10.RData")
...
#1
GDP10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GDP"), model = i)
})
GDP10 <- do.call(rbind, GDP10)

#2
Elevation10 <- lapply(1:length(model_list), function(i){
  data.frame(getVar(model_list[[i]],"Elevation"), model = i)
})
Elevation10 <- do.call(rbind, Elevation10)

#3
Barren10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Barren"), model = i)
})
Barren10 <- do.call(rbind, Barren10)

#4
PRS10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS10 <- do.call(rbind, PRS10)

#5
MammalRichness10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"MammalRichness"), model = i)
})
MammalRichness10 <- do.call(rbind, MammalRichness10)

#6
SSD10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD10 <- do.call(rbind, SSD10)

#7
PRE10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE10 <- do.call(rbind, PRE10)

#8
WIN10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN10 <- do.call(rbind, WIN10)

#9
RHU10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU10 <- do.call(rbind, RHU10)

#10
GST10 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST10 <- do.call(rbind, GST10)


write.csv(GDP10,"BRT/OUTPUT/HFRS/VAR/GDP10.csv")
write.csv(Elevation10,"BRT/OUTPUT/HFRS/VAR/Elevation10.csv")
write.csv(Barren10,"BRT/OUTPUT/HFRS/VAR/Barren10.csv")
write.csv(PRS10,"BRT/OUTPUT/HFRS/VAR/PRS10.csv")
write.csv(MammalRichness10,"BRT/OUTPUT/HFRS/VAR/MammalRichness10.csv")
write.csv(SSD10,"BRT/OUTPUT/HFRS/VAR/SSD10.csv")
write.csv(PRE10,"BRT/OUTPUT/HFRS/VAR/PRE10.csv")
write.csv(WIN10,"BRT/OUTPUT/HFRS/VAR/WIN10.csv")
write.csv(RHU10,"BRT/OUTPUT/HFRS/VAR/RHU10.csv")
write.csv(GST10,"BRT/OUTPUT/HFRS/VAR/GST10.csv")
