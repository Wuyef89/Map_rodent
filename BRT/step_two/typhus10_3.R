...
write.csv(test_table,"BRT/OUTPUT/TYPHUS/TEST/test3.csv")
...
writeRaster(typhus.risk, 
            filename = "BRT/OUTPUT/TYPHUS/TIF/typhus_risk3.tif", overwrite=TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/TYPHUS/CONTRIBUTION/contribution3.csv")
save.image("BRT/OUTPUT/TYPHUS/IMAGE/image3.RData")
...
Elevation3 <- do.call(cbind, Elevation3)
...
MammalRichness3 <- do.call(cbind, MammalRichness3)

#4
SSD3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD3 <- do.call(cbind, SSD3)

#5
PRS3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS3 <- do.call(cbind, PRS3)

#6
PRE3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE3 <- do.call(cbind, PRE3)

#7
RHU3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU3 <- do.call(cbind, RHU3)

#8
GST3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST3 <- do.call(cbind, GST3)

#9
EBT3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"EBT"), model = i)
})
EBT3 <- do.call(cbind, EBT3)

#10
WIN3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN3 <- do.call(cbind, WIN3)

#11
Shrubs3 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Shrubs"), model = i)
})
Shrubs3 <- do.call(cbind, Shrubs3)


write.csv(Elevation3,"BRT/OUTPUT/TYPHUS/VAR//Elevation3.csv")
write.csv(GDP3,"BRT/OUTPUT/TYPHUS/VAR//GDP3.csv")
write.csv(MammalRichness3,"BRT/OUTPUT/TYPHUS/VAR//MammalRichness3.csv")
write.csv(SSD3,"BRT/OUTPUT/TYPHUS/VAR//SSD3.csv")
write.csv(PRS3,"BRT/OUTPUT/TYPHUS/VAR//PRS3.csv")
write.csv(PRE3,"BRT/OUTPUT/TYPHUS/VAR//PRE3.csv")
write.csv(RHU3,"BRT/OUTPUT/TYPHUS/VAR//RHU3.csv")
write.csv(GST3,"BRT/OUTPUT/TYPHUS/VAR//GST3.csv")
write.csv(EBT3,"BRT/OUTPUT/TYPHUS/VAR//EBT3.csv")
write.csv(WIN3,"BRT/OUTPUT/TYPHUS/VAR//WIN3.csv")
write.csv(Shrubs3,"BRT/OUTPUT/TYPHUS/VAR//Shrubs3.csv")
