...
write.csv(test_table,"BRT/OUTPUT/TYPHUS/TEST/test6.csv")
...
writeRaster(typhus.risk, 
            filename = "BRT/OUTPUT/TYPHUS/TIF/typhus_risk6.tif", overwrite=TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/TYPHUS/CONTRIBUTION/contribution6.csv")
save.image("BRT/OUTPUT/TYPHUS/IMAGE/image6.RData")
...
Elevation6 <- do.call(cbind, Elevation6)
...
MammalRichness6 <- do.call(cbind, MammalRichness6)

#4
SSD6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD6 <- do.call(cbind, SSD6)

#5
PRS6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS6 <- do.call(cbind, PRS6)

#6
PRE6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE6 <- do.call(cbind, PRE6)

#7
RHU6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU6 <- do.call(cbind, RHU6)

#8
GST6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST6 <- do.call(cbind, GST6)

#9
EBT6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"EBT"), model = i)
})
EBT6 <- do.call(cbind, EBT6)

#10
WIN6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN6 <- do.call(cbind, WIN6)

#11
Shrubs6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Shrubs"), model = i)
})
Shrubs6 <- do.call(cbind, Shrubs6)


write.csv(Elevation6,"BRT/OUTPUT/TYPHUS/VAR//Elevation6.csv")
write.csv(GDP6,"BRT/OUTPUT/TYPHUS/VAR//GDP6.csv")
write.csv(MammalRichness6,"BRT/OUTPUT/TYPHUS/VAR//MammalRichness6.csv")
write.csv(SSD6,"BRT/OUTPUT/TYPHUS/VAR//SSD6.csv")
write.csv(PRS6,"BRT/OUTPUT/TYPHUS/VAR//PRS6.csv")
write.csv(PRE6,"BRT/OUTPUT/TYPHUS/VAR//PRE6.csv")
write.csv(RHU6,"BRT/OUTPUT/TYPHUS/VAR//RHU6.csv")
write.csv(GST6,"BRT/OUTPUT/TYPHUS/VAR//GST6.csv")
write.csv(EBT6,"BRT/OUTPUT/TYPHUS/VAR//EBT6.csv")
write.csv(WIN6,"BRT/OUTPUT/TYPHUS/VAR//WIN6.csv")
write.csv(Shrubs6,"BRT/OUTPUT/TYPHUS/VAR//Shrubs6.csv")
