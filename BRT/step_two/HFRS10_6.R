...
write.csv(test_table, "BRT/OUTPUT/HFRS/TEST/test6.csv")
...
writeRaster(HFRS.risk, 
            filename = "BRT/OUTPUT/HFRS/TIF/HFRS_risk6.tif", overwrite = TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution6.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image6.RData")
...
#1
GDP6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GDP"), model = i)
})
GDP6 <- do.call(rbind, GDP6)

#2
Elevation6 <- lapply(1:length(model_list), function(i){
  data.frame(getVar(model_list[[i]],"Elevation"), model = i)
})
Elevation6 <- do.call(rbind, Elevation6)

#3
Barren6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Barren"), model = i)
})
Barren6 <- do.call(rbind, Barren6)

#4
PRS6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS6 <- do.call(rbind, PRS6)

#5
MammalRichness6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"MammalRichness"), model = i)
})
MammalRichness6 <- do.call(rbind, MammalRichness6)

#6
SSD6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD6 <- do.call(rbind, SSD6)

#7
PRE6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE6 <- do.call(rbind, PRE6)

#8
WIN6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN6 <- do.call(rbind, WIN6)

#9
RHU6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU6 <- do.call(rbind, RHU6)

#6
GST6 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST6 <- do.call(rbind, GST6)


write.csv(GDP6,"BRT/OUTPUT/HFRS/VAR/GDP6.csv")
write.csv(Elevation6,"BRT/OUTPUT/HFRS/VAR/Elevation6.csv")
write.csv(Barren6,"BRT/OUTPUT/HFRS/VAR/Barren6.csv")
write.csv(PRS6,"BRT/OUTPUT/HFRS/VAR/PRS6.csv")
write.csv(MammalRichness6,"BRT/OUTPUT/HFRS/VAR/MammalRichness6.csv")
write.csv(SSD6,"BRT/OUTPUT/HFRS/VAR/SSD6.csv")
write.csv(PRE6,"BRT/OUTPUT/HFRS/VAR/PRE6.csv")
write.csv(WIN6,"BRT/OUTPUT/HFRS/VAR/WIN6.csv")
write.csv(RHU6,"BRT/OUTPUT/HFRS/VAR/RHU6.csv")
write.csv(GST6,"BRT/OUTPUT/HFRS/VAR/GST6.csv")
