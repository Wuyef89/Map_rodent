...
write.csv(test_table,"BRT/OUTPUT/TYPHUS/TEST/test1.csv")
...
writeRaster(typhus.risk, 
            filename = "BRT/OUTPUT/TYPHUS/TIF/typhus_risk1.tif", overwrite=TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/TYPHUS/CONTRIBUTION/contribution1.csv")
save.image("BRT/OUTPUT/TYPHUS/IMAGE/image1.RData")
...
Elevation1 <- do.call(cbind, Elevation1)
...
MammalRichness1 <- do.call(cbind, MammalRichness1)

#4
SSD1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD1 <- do.call(cbind, SSD1)

#5
PRS1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS1 <- do.call(cbind, PRS1)

#6
PRE1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE1 <- do.call(cbind, PRE1)

#7
RHU1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU1 <- do.call(cbind, RHU1)

#8
GST1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST1 <- do.call(cbind, GST1)

#9
EBT1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"EBT"), model = i)
})
EBT1 <- do.call(cbind, EBT1)

#10
WIN1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN1 <- do.call(cbind, WIN1)

#11
Shrubs1 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Shrubs"), model = i)
})
Shrubs1 <- do.call(cbind, Shrubs1)


write.csv(Elevation1,"BRT/OUTPUT/TYPHUS/VAR//Elevation1.csv")
write.csv(GDP1,"BRT/OUTPUT/TYPHUS/VAR//GDP1.csv")
write.csv(MammalRichness1,"BRT/OUTPUT/TYPHUS/VAR//MammalRichness1.csv")
write.csv(SSD1,"BRT/OUTPUT/TYPHUS/VAR//SSD1.csv")
write.csv(PRS1,"BRT/OUTPUT/TYPHUS/VAR//PRS1.csv")
write.csv(PRE1,"BRT/OUTPUT/TYPHUS/VAR//PRE1.csv")
write.csv(RHU1,"BRT/OUTPUT/TYPHUS/VAR//RHU1.csv")
write.csv(GST1,"BRT/OUTPUT/TYPHUS/VAR//GST1.csv")
write.csv(EBT1,"BRT/OUTPUT/TYPHUS/VAR//EBT1.csv")
write.csv(WIN1,"BRT/OUTPUT/TYPHUS/VAR//WIN1.csv")
write.csv(Shrubs1,"BRT/OUTPUT/TYPHUS/VAR//Shrubs1.csv")
