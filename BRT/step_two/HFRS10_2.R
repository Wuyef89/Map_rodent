...
write.csv(test_table, "BRT/OUTPUT/HFRS/TEST/test2.csv")
...
writeRaster(HFRS.risk, 
            filename = "BRT/OUTPUT/HFRS/TIF/HFRS_risk2.tif", overwrite = TRUE)
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution2.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image2.RData")
...
Elevation2 <- do.call(rbind, Elevation2)

#3
Barren2 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"Barren"), model = i)
})
Barren2 <- do.call(rbind, Barren2)

#4
PRS2 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRS"), model = i)
})
PRS2 <- do.call(rbind, PRS2)

#5
MammalRichness2 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"MammalRichness"), model = i)
})
MammalRichness2 <- do.call(rbind, MammalRichness2)

#6
SSD2 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"SSD"), model = i)
})
SSD2 <- do.call(rbind, SSD2)

#7
PRE2 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"PRE"), model = i)
})
PRE2 <- do.call(rbind, PRE2)

#8
WIN2 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"WIN"), model = i)
})
WIN2 <- do.call(rbind, WIN2)

#9
RHU2 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"RHU"), model = i)
})
RHU2 <- do.call(rbind, RHU2)

#10
GST2 <- lapply(1:length(model_list),function(i){
  data.frame(getVar(model_list[[i]],"GST"), model = i)
})
GST2 <- do.call(rbind, GST2)


write.csv(GDP2,"BRT/OUTPUT/HFRS/VAR/GDP2.csv")
write.csv(Elevation2,"BRT/OUTPUT/HFRS/VAR/Elevation2.csv")
write.csv(Barren2,"BRT/OUTPUT/HFRS/VAR/Barren2.csv")
write.csv(PRS2,"BRT/OUTPUT/HFRS/VAR/PRS2.csv")
write.csv(MammalRichness2,"BRT/OUTPUT/HFRS/VAR/MammalRichness2.csv")
write.csv(SSD2,"BRT/OUTPUT/HFRS/VAR/SSD2.csv")
write.csv(PRE2,"BRT/OUTPUT/HFRS/VAR/PRE2.csv")
write.csv(WIN2,"BRT/OUTPUT/HFRS/VAR/WIN2.csv")
write.csv(RHU2,"BRT/OUTPUT/HFRS/VAR/RHU2.csv")
write.csv(GST2,"BRT/OUTPUT/HFRS/VAR/GST2.csv")
