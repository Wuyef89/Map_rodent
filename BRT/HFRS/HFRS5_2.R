...
# Read the case data corresponding to the province
...
scv1 <- cv_spatial(
  ...
  progress = FALSE, 
  biomod2 = TRUE, 
  raster_colors = terrain.colors(5, rev = TRUE) 
) 

# Extract data from the rasters object at the points' locations
...
for (k in seq_len(length(folds))) {  
  ...
  model_list[[k]] <- gbm.step(model_data[trainSet, ], gbm.x = 1:24, gbm.y = 25, family = "bernoulli", tree.complexity = 5,
...
write.csv(contribution.all, "BRT/OUTPUT/HFRS/CONTRIBUTION/contribution5_2.csv")
save.image("BRT/OUTPUT/HFRS/IMAGE/image5_2.RData")
